...
button.addEventListener("click", (event) => { 
 ...
 // Lançar um alerta, caso o item tenha menos que 3 letras  window.alert("O item deve ter, pelo menos, 3 letras.");  } else { 
 ...
 // Criando uma span para inserir o texto do produto da lista  const textItem = document.createElement("span"); 
 // Criando um botão para remover o produto da lista 
 const removeButtonItem = document.createElement("button"); 
 // Adicionando o texto que o usuário digitou no texto do item (produto)  textItem.textContent = input.value; 
 // Adicionando um novo atributo ao checkbox 
 checkboxItem.setAttribute("type", "checkbox"); 
 // Adicionando texto "Remover" no botão 
 removeButtonItem.textContent = "Remover"; 
 // Inserindo o checkbox, o texto e o botão de remover na li (novoItem)  novoItem.insertAdjacentElement("beforeend", checkboxItem);  novoItem.insertAdjacentElement("beforeend", textItem); 
 ...
 // Limpando o conteúdo do input quando o item é adicionado  input.value = ""; 
 } 
}); 
